...
class StableDiffusion:
    def __init__(self, model_cfg):
        ...
        self.device = torch.device(
            f"cuda:{model_cfg['gpu_id']}" if torch.cuda.is_available() else "cpu"
        )
        ...
        self.diffusion_steps = (
            model_cfg["diffusion_steps"] if "diffusion_steps" in model_cfg else 50
        )
        self.guidance_scale = (
            model_cfg["guidance_scale"] if "guidance_scale" in model_cfg else 7.5
        )

        # Prompt parameters
        self.prompt = model_cfg["prompt"] if "prompt" in model_cfg else None
        self.load_prompt_embeds = (
            model_cfg["load_prompt_embeds"]
            if "load_prompt_embeds" in model_cfg
            else None
        )
        ...
        self.rand_seed = (
            model_cfg["rand_seed"]
            if "rand_seed" in model_cfg
            else random.randint(0, 10**6)
        )
        self.images_per_prompt = (
            model_cfg["images_per_prompt"] if "images_per_prompt" in model_cfg else 1
        )
        self.height = model_cfg["height"] if "height" in model_cfg else None
        self.width = model_cfg["width"] if "width" in model_cfg else None
        self.load_latent_noise = (
            model_cfg["load_latent_noise"] if "load_latent_noise" in model_cfg else None
        )
    ...
    def _load_sd_pipeline(
        ...
        """
        print(
            f"Loading Stable Diffusion {self.model_identifier} from the diffusers (HuggingFace) library.."
        )
    ...
    def _load_scheduler(self, scheduler_name, model_id):
        ...
        if scheduler_name == "EulerDiscreteScheduler":
            scheduler = diffusers.EulerDiscreteScheduler.from_pretrained(
        ...
        if scheduler_name == "HeunDiscreteScheduler":
            scheduler = diffusers.HeunDiscreteSchedule.from_pretrained(
                model_id, subfolder="scheduler"
            )
        if scheduler_name == "DEISMultistepScheduler":
            scheduler = diffusers.DEISMultistepScheduler.from_pretrained(
                model_id, subfolder="scheduler"
            )
        ...
        if scheduler_name == "LMSDiscreteScheduler":
            scheduler = diffusers.LMSDiscreteScheduler.from_pretrained(
                model_id, subfolder="scheduler"
            )
        ...
        if scheduler_name == "KDPM2AncestralDiscreteScheduler":
            scheduler = diffusers.KDPM2AncestralDiscreteScheduler.from_pretrained(
                model_id, subfolder="scheduler"
            )
        ...
        if scheduler_name == "PNDMScheduler":
            scheduler = diffusers.PNDMScheduler.from_pretrained(
                model_id, subfolder="scheduler"
            )
        if scheduler_name == "KDPM2DiscreteScheduler":
            scheduler = diffusers.KDPM2DiscreteScheduler.from_pretrained(
                model_id, subfolder="scheduler"
            )
    ...
    def sample_noise(self, height, width, images_per_prompt, rand_seed):
        ...
        shape = (
            images_per_prompt,
            self.sd_pipeline.vae.latent_channels,
            height // scale_factor,
            width // scale_factor,
        )
    ...
    def decode_images(self, images):
        ...
        images = self.sd_pipeline.vae.decode(images.to(MODEL_DTYPE)).sample
    ...
    def load_noise(
        self, load_latent_noise, height, width, images_per_prompt, rand_seed
    ):
        """
        ...
        if os.path.isfile(load_latent_noise):
            ...
            scale_factor = 2 ** (
                len(self.sd_pipeline.vae.config.block_out_channels) - 1
            )
    ...
    def load_prompt(self, load_prompt_embeds, prompt):
        ...
        if os.path.isfile(load_prompt_embeds):
            ...
            prompt_embed = torch.load(load_prompt_embeds)["prompt_embed"].to(
                self.device
            )
            return prompt_embed
        else:
...
class Txt2Img(StableDiffusion):
    ...
    def run_sd_inference(
        ...
        self.sd_pipeline.scheduler.set_timesteps(
            self.diffusion_steps, device=self.device
        )
        ...
        # Diffusion loop
        results = [
            [] for _ in range(latent_noise.shape[0])
        ]  # Stores the results of each diffusion step
        for t in tqdm(self.sd_pipeline.scheduler.timesteps):
            ...
            latent_model_input = self.sd_pipeline.scheduler.scale_model_input(
            ...
            noise_pred = self.sd_pipeline.unet(
                ...
                t.to(MODEL_DTYPE),
                encoder_hidden_states=prompt_embed.to(MODEL_DTYPE),
            ).sample
            ...
            latent_noise = self.sd_pipeline.scheduler.step(
                noise_pred, t, latent_noise
            ).prev_sample
        ...
        ]  # Placeholder for all processed image embeddings
        images = [
            [] for _ in range(len(results))
        ]  # Placeholder for all decoded image embeddings
        for batch_idx in range(len(results)):
            for diffusion_step in range(len(results[batch_idx])):
                if (
                    not visualize_diffusion
                    and diffusion_step != len(results[batch_idx]) - 1
                ):
                    # Skip all diffusion steps except the last one
                    continue
                img_embeds[batch_idx].append(
                    results[batch_idx][diffusion_step]
                    / self.sd_pipeline.vae.config.scaling_factor
                )
                images[batch_idx].append(
                    self.decode_images(img_embeds[batch_idx][-1])[0]
                )
...
class Img2Img(StableDiffusion):
    ...
    @torch.no_grad()
    def run_sd_inference(
        self, prompt_embed, latent_noise, image, mask=None, visualize_diffusion=False
    ):
        """
        ...
        timesteps, num_diffusion_steps = self.sd_pipeline.get_timesteps(
            self.diffusion_steps, self.strength, self.device
        )
        ...
        latent_noise = self.sd_pipeline.scheduler.add_noise(
            encoded_imgs, latent_noise, latent_timestep
        )
...
class Inpaint(StableDiffusion):
    ...
    @torch.no_grad()
    def run_sd_inference(
        self, prompt_embed, latent_noise, image, mask, visualize_diffusion=False
    ):
        """
        ...
        mask, masked_image = (
            diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_inpaint.prepare_mask_and_masked_image(
                image, mask
            )
        )
